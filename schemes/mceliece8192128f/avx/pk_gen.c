...
static int mov_columns(uint64_t mat[][ 128 ], uint32_t *perm) {
    ...
    for (i = 0; i < GFBITS * SYS_T; i += 64) {
        ...
        for (j = 0; j < min(64, GFBITS * SYS_T - i); j++) {
            mat[ i + j ][ block_idx + 0 ] = (mat[ i + j ][ block_idx + 0 ] & 0x00000000FFFFFFFF) | (buf[j] << 32);
            mat[ i + j ][ block_idx + 1 ] = (mat[ i + j ][ block_idx + 1 ] & 0xFFFFFFFF00000000) | (buf[j] >> 32);
        }
    }
