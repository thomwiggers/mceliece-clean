...
void MC_vec128_sq(vec128 *out, const vec128 *in) {
    ...
    t = MC_vec128_xor(in[11], in[12]);

    result[0] = MC_vec128_xor(in[0], in[11]);
    result[1] = MC_vec128_xor(in[7], t);
    result[2] = MC_vec128_xor(in[1], in[7]);
    result[3] = MC_vec128_xor(in[8], t);
    result[4] = MC_vec128_xor(in[2], in[7]);
    result[4] = MC_vec128_xor(result[4], in[8]);
    result[4] = MC_vec128_xor(result[4], t);
    result[5] = MC_vec128_xor(in[7], in[9]);
    result[6] = MC_vec128_xor(in[3], in[8]);
    result[6] = MC_vec128_xor(result[6], in[9]);
    result[6] = MC_vec128_xor(result[6], in[12]);
    result[7] = MC_vec128_xor(in[8], in[10]);
    result[8] = MC_vec128_xor(in[4], in[9]);
    result[8] = MC_vec128_xor(result[8], in[10]);
    result[9] = MC_vec128_xor(in[9], in[11]);
    result[10] = MC_vec128_xor(in[5], in[10]);
    result[10] = MC_vec128_xor(result[10], in[11]);
    result[11] = MC_vec128_xor(in[10], in[12]);
    result[12] = MC_vec128_xor(in[6], t);
...
void MC_vec128_inv(vec128 *out, const vec128 *in) {
    ...
    MC_vec128_copy(out, in);

    MC_vec128_sq(out, out);
    MC_vec128_mul(tmp_11, out, in); // ^11

    MC_vec128_sq(out, tmp_11);
    MC_vec128_sq(out, out);
    MC_vec128_mul(tmp_1111, out, tmp_11); // ^1111

    MC_vec128_sq(out, tmp_1111);
    MC_vec128_sq(out, out);
    MC_vec128_sq(out, out);
    MC_vec128_sq(out, out);
    MC_vec128_mul(out, out, tmp_1111); // ^11111111

    MC_vec128_sq(out, out);
    MC_vec128_sq(out, out);
    MC_vec128_sq(out, out);
    MC_vec128_sq(out, out);
    MC_vec128_mul(out, out, tmp_1111); // ^111111111111

    MC_vec128_sq(out, out); // ^1111111111110
}
