...
void MC_bm(vec *out, vec in[][ GFBITS ]) {
    uint16_t i;
    uint16_t N, L;
    ...
    for (N = 0; N < SYS_T * 2; N++) {
        ...
        for (i = 0; i < GFBITS; i++) {
            d_vec[i] = MC_vec_setbits((d >> i) & 1);
            b_vec[i] = MC_vec_setbits((b >> i) & 1);
        }
    ...
    for (i = 0; i < GFBITS; i++) {
        out[i] = MC_vec_setbits((c0 >> i) & 1);
    }

    MC_vec_mul(out, out, C);
}
