...
#include "consts.inc"
    ...
    for (j = 0; j < 64; j += 4) {
        for (i = 0; i < GFBITS; i++) {
            ...
            out[j / 2 + 0][i] = MC_vec128_set2x(t0, t1);
            ...
            out[j / 2 + 1][i] = MC_vec128_set2x(t0, t1);
        }
    }
    ...
    for (i = 0; i < 32; i += 2) {
        MC_vec128_mul(tmp, out[i + 1], consts[ 0 ]);
        ...
        for (b = 0; b < GFBITS; b++) {
            x[b] = MC_vec128_unpack_low(out[i + 0][b], out[i + 1][b]);
        }
        for (b = 0; b < GFBITS; b++) {
            y[b] = MC_vec128_unpack_high(out[i + 0][b], out[i + 1][b]);
        }
    ...
    for (i = 0; i <= 4; i++) {
        ...
        for (j = 0; j < 32; j += 2 * s) {
            for (k = j; k < j + s; k++) {
                MC_vec128_mul(tmp, out[k + s], consts[ consts_ptr + (k - j) ]);
...
void MC_fft(vec128 out[][ GFBITS ], uint64_t *in) {
