...
#include "consts.inc"
    ...
    for (i = 0; i < 32; i += 2) {
        for (b = 0; b < GFBITS; b++) {
            x[b] = vec128_unpack_low(in[i + 0][b], in[i + 1][b]);
        }
        for (b = 0; b < GFBITS; b++) {
            y[b] = vec128_unpack_high(in[i + 0][b], in[i + 1][b]);
        }
