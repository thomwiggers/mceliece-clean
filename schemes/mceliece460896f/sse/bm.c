...
static inline void vec128_cmov(vec128 *out, vec128 *in, uint16_t mask) {
    ...
    vec128 m0 = MC_vec128_set1_16b( mask);
    vec128 m1 = MC_vec128_set1_16b(~mask);

    for (i = 0; i < GFBITS; i++) {
        v0 = MC_vec128_and(in[i], m0);
        v1 = MC_vec128_and(out[i], m1);
        out[i] = MC_vec128_or(v0, v1);
    }
}
...
static inline void interleave(vec128 *in, int idx0, int idx1, vec128 *mask, int b) {
    ...
    x = MC_vec128_or(MC_vec128_and(in[idx0], mask[0]),
                  MC_vec128_sll_2x(MC_vec128_and(in[idx1], mask[0]), s));

    y = MC_vec128_or(MC_vec128_srl_2x(MC_vec128_and(in[idx0], mask[1]), s),
                  MC_vec128_and(in[idx1], mask[1]));
...
static inline void get_coefs(gf *out, vec128 *in) {
    ...
    for (i = 13; i < 16; i++) {
        buf[i] = MC_vec128_setzero();
    }

    mask[0][0] = MC_vec128_set1_16b(0x5555);
    mask[0][1] = MC_vec128_set1_16b(0xAAAA);
    mask[1][0] = MC_vec128_set1_16b(0x3333);
    mask[1][1] = MC_vec128_set1_16b(0xCCCC);
    mask[2][0] = MC_vec128_set1_16b(0x0F0F);
    mask[2][1] = MC_vec128_set1_16b(0xF0F0);
    mask[3][0] = MC_vec128_set1_16b(0x00FF);
    mask[3][1] = MC_vec128_set1_16b(0xFF00);
    ...
    for (i = 0; i < 16; i++)
        for (k = 0; k <  4; k++) {
            out[ (4 * 0 + k) * 16 + i ] = (MC_vec128_extract(buf[i], 0) >> (k * 16)) & GFMASK;
            out[ (4 * 1 + k) * 16 + i ] = (MC_vec128_extract(buf[i], 1) >> (k * 16)) & GFMASK;
        }
}
...
void MC_bm(vec128 *out, vec128 in[][ GFBITS ]) {
    ...
    C[0] = MC_vec128_set2x(0, one << 63);
    B[0] = MC_vec128_set2x(0, one << 62);

    for (i = 1; i < GFBITS; i++) {
        C[i] = B[i] = MC_vec128_setzero();
    }
    ...
    for (i = 0; i < GFBITS; i++) {
        interval[i] = MC_vec128_setzero();
    }

    for (N = 0; N < SYS_T * 2; N++) {
        MC_update_asm(interval, coefs[N]);
        MC_vec128_mul(prod, C, (vec128 *) interval);
        d = MC_vec_reduce_asm(prod);

        mask = mask_nonzero(d) & mask_leq(L * 2, N);

        for (i = 0; i < GFBITS; i++) {
            dd[i] = MC_vec128_setbits((d >> i) & 1);
            bb[i] = MC_vec128_setbits((b >> i) & 1);
        }

        MC_vec128_mul(B_tmp, dd, B);
        MC_vec128_mul(C_tmp, bb, C);

        vec128_cmov(B, C, mask);
        MC_update_asm(B, 0);

        for (i = 0; i < GFBITS; i++) {
            C[i] = MC_vec128_xor(B_tmp[i], C_tmp[i]);
        }
    ...
    for (i = 0; i < GFBITS; i++) {
        v[0] = MC_vec128_extract(C[i], 0);
        v[1] = MC_vec128_extract(C[i], 1);

        out[i] = MC_vec128_set2x((v[0] >> 31) | (v[1] << 33), v[1] >> 31);
    }
}
